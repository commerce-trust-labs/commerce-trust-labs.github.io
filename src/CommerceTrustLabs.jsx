...
const GlobalStyle = () => (
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .ch{transition:background .25s;cursor:default}
    ...
    .bg{display:inline-flex;align-items:center;gap:.5rem;background:transparent;color:#e8e4dc;padding:.8rem 1.6rem;font-family:'DM Mono',monospace;font-size:.72rem;letter-spacing:.14em;text-transform:uppercase;text-decoration:none;border:1px solid rgba(232,228,220,.1);transition:border-color .2s}
    .bg:hover{border-color:rgba(232,228,220,.3)}
...
const W = { maxWidth:"1200px", margin:"0 auto", padding:"0 clamp(1.5rem,4vw,4rem)" };
const S = { padding:"6rem 0" };
...
const Reveal = ({children, delay=0}) => {
    ...
    return <div ref={ref} style={{opacity:v?1:0, transform:v?"none":"translateY(20px)", transition:`opacity .7s ease ${delay}ms, transform .7s ease ${delay}ms`}}>{children}</div>;
};
...
const Nav = () => {
    ...
    useEffect(() => {
        ...
        const o = new IntersectionObserver(es => { es.forEach(e => { if(e.isIntersecting) setActive(e.target.id); }); }, {threshold:0.3});
        ids.forEach(id => { const el = document.getElementById(id); if(el) o.observe(el); });
        return () => o.disconnect();
    }, []);
    return (
        <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:200,display:"flex",justifyContent:"space-between",alignItems:"center",padding:`1rem clamp(1.5rem,4vw,4rem)`,background:solid?"rgba(11,15,20,.97)":"rgba(11,15,20,.5)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${solid?C.border:"transparent"}`,transition:"all .3s"}}>
    ...
    return (
        <div style={{background:C.accent,overflow:"hidden",padding:".55rem 0",whiteSpace:"nowrap"}}>
            <div style={{display:"inline-flex",gap:"3rem",animation:"ticker 28s linear infinite"}}>
                {[...items,...items].map((t,i)=><span key={i} style={{fontFamily:"'DM Mono',monospace",fontSize:".68rem",letterSpacing:".2em",textTransform:"uppercase",color:"rgba(255,255,255,.9)"}}>{t}</span>)}
            </div>
        </div>
    );
};
...
/* NETWORK */
const Network = () => (
    <svg viewBox="0 0 500 500" fill="none" style={{width:"100%",maxWidth:"440px",display:"block"}}>
        <defs><radialGradient id="rg" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="#c4501a" stopOpacity="0.12"/><stop offset="100%" stopColor="#c4501a" stopOpacity="0"/></radialGradient></defs>
        ...
        {[[255,62,"#c4501a","POLICY ENGINE"],[424,165,"#6fa3d8","COMPLIANCE"],[424,342,"#6fa3d8","SETTLEMENT"],[180,450,"#7a8494","ANALYTICS"],[2,342,"#c4501a","GOVERNANCE"],[2,165,"#6fa3d8","TRUST LAYER"]].map(([x,y,col,label],i)=><text key={i} x={x} y={y} fontFamily="'DM Mono',monospace" fontSize="8" fill={col} opacity="0.8">{label}</text>)}
    </svg>
);
...
const Hero = () => {
    ...
    return (
        <section style={{background:C.bg,paddingTop:"68px",minHeight:"100vh",display:"flex",alignItems:"center",overflow:"hidden"}}>
            <div style={{...W,width:"100%",display:"flex",gap:"4rem",alignItems:"center",flexWrap:"wrap",padding:"4rem clamp(1.5rem,4vw,4rem)"}}>
                {/* left */}
                <div style={{flex:"1 1 380px",minWidth:0}}>
                    ...
                    <h1 style={{...a(160),fontFamily:"'Syne',sans-serif",fontSize:"clamp(2.2rem,3.8vw,4.2rem)",fontWeight:800,lineHeight:1.0,letterSpacing:"-.03em",color:C.text,marginBottom:"1.5rem"}}>
                    ...
                    <p style={{...a(260),fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:".84rem",lineHeight:1.85,color:C.muted,maxWidth:"40ch",marginBottom:"2.5rem"}}>
                ...
                {/* right */}
                <div style={{flex:"1 1 340px",minWidth:0,display:"flex",justifyContent:"center",position:"relative"}}>
...
const Problem = () => {
    ...
    return (
        <section id="problem" style={{background:C.bg2,...S}}>
            <div style={W}>
                <Label light>The Problem</Label>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:"4rem",alignItems:"start"}}>
                    <div>
                        <Reveal>
                            <h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.8rem,2.8vw,2.8rem)",fontWeight:800,lineHeight:1.05,letterSpacing:"-.03em",color:C.text}}>
...
const Mission = () => {
    ...
    return (
        <section id="mission" style={{background:C.bg,...S}}>
            <div style={W}>
                <Label>Mission & Vision</Label>
                <Reveal>
                    <p style={{fontFamily:"'Instrument Serif',serif",fontStyle:"italic",fontSize:"clamp(1.3rem,2.2vw,2rem)",lineHeight:1.4,color:C.text,maxWidth:"30ch",marginBottom:"4rem",paddingLeft:"1rem",borderLeft:`3px solid ${C.accent}`}}>
...
const Technology = () => {
    ...
    return (
        <section id="technology" style={{background:C.bg3,...S}}>
            <div style={W}>
                <Label light>Reference Architecture</Label>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:"2.5rem",alignItems:"end",marginBottom:"3rem"}}>
                    <Reveal>
                        <h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.6rem,2.5vw,2.4rem)",fontWeight:800,lineHeight:1.05,letterSpacing:"-.03em",color:C.text}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:"1px",background:C.border,border:`1px solid ${C.border}`}}>
                    {cards.map((c,i)=>(
                        <Reveal key={i} delay={i*80}>
...
const Interest = () => {
    ...
    return (
        <section id="interest" style={{background:C.bg,...S}}>
            <div style={W}>
                <Label>Why This Matters</Label>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:"4rem",alignItems:"start"}}>
                    <div>
                        <Reveal>
                            <h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.6rem,2.5vw,2.6rem)",fontWeight:800,lineHeight:1.05,letterSpacing:"-.03em",color:C.text,marginBottom:"1.5rem"}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:"1rem"}}>
                        {cards.map((c,i)=>(
                            <Reveal key={i} delay={i*90}>
                                <div className="ch" style={{background:C.bg2,border:`1px solid ${C.border}`,padding:"1.6rem 1.8rem"}}>
...
const Research = () => {
    ...
    return (
        <section id="research" style={{background:C.bg2,...S}}>
            <div style={W}>
                <Label light>Research & Publications</Label>
                <Reveal>
                    <h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.5rem,2.5vw,2.4rem)",fontWeight:800,letterSpacing:"-.03em",color:C.text,marginBottom:"3rem",lineHeight:1.05}}>
                    ...
                    {pubs.map((p,i)=>(
                        <Reveal key={i} delay={i*60}>
                            <div className="pr" style={{background:C.bg,padding:"1.6rem 2rem",display:"grid",gridTemplateColumns:"5rem 1fr auto",gap:"1.5rem",alignItems:"center"}}>
                                ...
                                {p.link?<a href={p.link} target="_blank" rel="noreferrer" className="lk">{p.linkLabel}</a>:<span style={{fontFamily:"'DM Mono',monospace",fontSize:".66rem",color:C.muted,whiteSpace:"nowrap"}}>{p.linkLabel}</span>}
                            </div>
                        </Reveal>
                    ))}
                </div>
...
/* TEAM */
const Team = () => (
    <section id="team" style={{background:C.bg,...S}}>
        <div style={W}>
            <Label>Founder</Label>
            <Reveal>
                <div style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:"4rem",alignItems:"start",border:`1px solid ${C.border}`,padding:"3rem"}}>
                    ...
                    <div style={{minWidth:0}}>
                        <h3 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.2rem,2vw,1.7rem)",fontWeight:800,lineHeight:1.1,letterSpacing:"-.03em",color:C.text,marginBottom:"1.8rem"}}>
                            Building the{" "}
                        ...
                        ].map((para,i)=><p key={i} style={{fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:".78rem",lineHeight:1.9,color:C.muted,marginBottom:"1rem"}}>{para}</p>)}
...
const Footer = () => (
    <footer style={{background:C.bg2,borderTop:`1px solid ${C.border}`}}>
        <div style={{...W,padding:"3rem clamp(1.5rem,4vw,4rem)",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:"2rem",alignItems:"end"}}>
            <div>
