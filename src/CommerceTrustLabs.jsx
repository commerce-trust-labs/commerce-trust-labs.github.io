...
const GlobalStyle = () => (
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    .nl { color:#7a8494; transition:color 0.2s; font-family:'DM Mono',monospace; font-size:0.65rem; letter-spacing:0.12em; text-transform:uppercase; text-decoration:none; }
    .nl:hover, .nl.active { color:#c4501a; }
    .ch { transition:background 0.25s; cursor:default; }
    .ch:hover { background:#161e28 !important; }
    .pr { transition:background 0.2s; }
    .pr:hover { background:#161e28 !important; }
    .bp { display:inline-flex; align-items:center; gap:0.6rem; background:#c4501a; color:#fff; padding:0.85rem 1.8rem; font-family:'DM Mono',monospace; font-size:0.72rem; letter-spacing:0.14em; text-transform:uppercase; text-decoration:none; transition:background 0.2s; }
    .bp:hover { background:#e06030; }
    .bg { display:inline-flex; align-items:center; gap:0.6rem; background:transparent; color:#e8e4dc; padding:0.85rem 1.8rem; font-family:'DM Mono',monospace; font-size:0.72rem; letter-spacing:0.14em; text-transform:uppercase; text-decoration:none; border:1px solid rgba(232,228,220,0.08); transition:border-color 0.2s; }
    .bg:hover { border-color:rgba(232,228,220,0.25); }
    .lk { opacity:0.4; transition:opacity 0.2s; text-decoration:none; font-family:'DM Mono',monospace; font-size:0.68rem; color:#e8e4dc; white-space:nowrap; }
    .lk:hover { opacity:1; }
    .fl { color:rgba(232,228,220,0.3); text-decoration:none; transition:color 0.2s; }
...
const SEC = { padding:"7rem 0" };
const INNER = { maxWidth:MAX, margin:"0 auto", padding:`0 ${PAD}` };
...
const Reveal = ({children,delay=0}) => {
    ...
    return <div ref={ref} style={{opacity:v?1:0,transform:v?"none":"translateY(22px)",transition:`opacity 0.8s cubic-bezier(0.16,1,0.3,1) ${delay}ms,transform 0.8s cubic-bezier(0.16,1,0.3,1) ${delay}ms`}}>{children}</div>;
};
...
const Nav = () => {
    ...
    return (
        <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:200,display:"flex",justifyContent:"space-between",alignItems:"center",padding:`1.1rem ${PAD}`,background:solid?"rgba(11,15,20,0.97)":"rgba(11,15,20,0.4)",backdropFilter:"blur(18px)",borderBottom:`1px solid ${solid?C.border:"transparent"}`,transition:"all 0.3s"}}>
            <a href="#" style={{fontFamily:"'DM Mono',monospace",fontSize:"0.7rem",letterSpacing:"0.2em",textTransform:"uppercase",color:C.text,textDecoration:"none"}}>Commerce Trust Labs</a>
            <div style={{display:"flex",gap:"2rem"}}>
                {links.map(([id,label])=><a key={id} href={`#${id}`} className={`nl${active===id?" active":""}`}>{label}</a>)}
            </div>
        </nav>
    );
};
    ...
    return (
        <div style={{background:C.accent,overflow:"hidden",padding:"0.6rem 0",whiteSpace:"nowrap"}}>
            <div style={{display:"inline-flex",gap:"3rem",animation:"ticker 28s linear infinite"}}>
                {[...items,...items].map((item,i)=><span key={i} style={{fontFamily:"'DM Mono',monospace",fontSize:"0.68rem",letterSpacing:"0.2em",textTransform:"uppercase",color:"rgba(255,255,255,0.9)"}}>{item}</span>)}
            </div>
        </div>
    );
};
...
/* ── NETWORK SVG ─── */
const Network = () => (
    <svg viewBox="0 0 500 500" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",maxWidth:"460px",display:"block"}}>
        <defs><radialGradient id="rg" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="#c4501a" stopOpacity="0.14"/><stop offset="100%" stopColor="#c4501a" stopOpacity="0"/></radialGradient></defs>
        ...
        {[[250,70],[415,170],[415,330],[250,430],[85,330],[85,170]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="5" fill={i%2===0?"#c4501a":"#6fa3d8"} opacity="0.9"/>)}
        {[[250,70,415,170],[415,170,415,330],[415,330,250,430],[250,430,85,330],[85,330,85,170],[85,170,250,70]].map(([x1,y1,x2,y2],i)=><line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#e8e4dc" strokeWidth="0.5" opacity="0.08"/>)}
        ...
        {[[250,165],[335,208],[335,293],[250,335],[165,293],[165,208]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="3" fill={i%2===0?"#c4501a":"#6fa3d8"} opacity="0.55"/>)}
        {[[250,165],[335,208],[335,293],[250,335],[165,293],[165,208]].map(([x,y],i)=><line key={i} x1="250" y1="250" x2={x} y2={y} stroke="#c4501a" strokeWidth="0.8" opacity="0.28"/>)}
        ...
        {[[255,62,"#c4501a","POLICY ENGINE"],[422,165,"#6fa3d8","COMPLIANCE"],[422,342,"#6fa3d8","SETTLEMENT"],[175,450,"#7a8494","ANALYTICS"],[2,342,"#c4501a","GOVERNANCE"],[2,165,"#6fa3d8","TRUST LAYER"]].map(([x,y,color,label],i)=><text key={i} x={x} y={y} fontFamily="'DM Mono',monospace" fontSize="8" fill={color} opacity="0.8">{label}</text>)}
    </svg>
);
...
const Hero = () => {
    ...
    const a=(d)=>({opacity:up?1:0,transform:up?"none":"translateY(18px)",transition:`opacity 0.7s ease ${d}ms,transform 0.7s ease ${d}ms`});
    return (
        <section style={{minHeight:"100vh",background:C.bg,paddingTop:"70px",overflow:"hidden"}}>
            <div style={{...INNER,minHeight:"calc(100vh - 70px)",display:"flex",alignItems:"center",gap:"4rem",flexWrap:"wrap"}}>
                {/* left */}
                <div style={{flex:"1 1 420px",minWidth:0,paddingTop:"2rem",paddingBottom:"2rem"}}>
                    <div style={{...a(80),display:"flex",alignItems:"center",gap:"0.8rem",marginBottom:"1.8rem"}}>
                    ...
                    <h1 style={{...a(160),fontFamily:"'Syne',sans-serif",fontSize:"clamp(2.4rem,4vw,4.6rem)",fontWeight:800,lineHeight:1.0,letterSpacing:"-0.03em",color:C.text,marginBottom:"1.6rem"}}>
                    ...
                    <p style={{...a(260),fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:"0.85rem",lineHeight:1.85,color:C.muted,maxWidth:"42ch",marginBottom:"2.5rem"}}>
                    ...
                    <div style={{...a(360),display:"flex",gap:"1rem",flexWrap:"wrap",marginBottom:"4rem"}}>
                        <a href="#mission" className="bp">Our Mission →</a>
                        <a href="https://github.com/commerce-trust-labs" target="_blank" rel="noreferrer" className="bg">GitHub →</a>
                    </div>
                    <div style={{...a(460),display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1px",background:C.border,border:`1px solid ${C.border}`}}>
                ...
                {/* right */}
                <div style={{flex:"1 1 380px",minWidth:0,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",padding:"2rem 0"}}>
...
const Problem = () => {
    ...
    return (
        <section id="problem" style={{background:C.bg2,...SEC}}>
            <div style={INNER}>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:"5rem",alignItems:"start"}}>
                    <div>
                        <Label light>The Problem</Label>
                        <Reveal><h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.8rem,3vw,3rem)",fontWeight:800,lineHeight:1.05,letterSpacing:"-0.03em",color:C.text}}>
                        ...
                        <Reveal delay={180}><p style={{fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:"0.8rem",lineHeight:1.85,color:C.muted,marginTop:"2rem"}}>The U.S. retail economy is rapidly adopting AI — but without the governance infrastructure needed to ensure these systems operate safely, fairly, and in compliance with national standards.</p></Reveal>
                    </div>
...
const Mission = () => {
    ...
    return (
        <section id="mission" style={{background:C.bg,...SEC}}>
            <div style={INNER}>
                <Label>Mission & Vision</Label>
                <Reveal>
                    <p style={{fontFamily:"'Instrument Serif',serif",fontStyle:"italic",fontSize:"clamp(1.5rem,2.5vw,2.4rem)",lineHeight:1.35,color:C.text,maxWidth:"26ch",marginBottom:"5rem",position:"relative"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:"1px",background:C.border,border:`1px solid ${C.border}`}}>
                    {pillars.map((p,i)=>(
                        <Reveal key={i} delay={i*110}>
                            <div className="ch" style={{background:C.bg2,padding:"2.5rem",height:"100%"}}>
...
const Technology = () => {
    ...
    return (
        <section id="technology" style={{background:C.bg3,...SEC}}>
            <div style={INNER}>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:"3rem",alignItems:"end",marginBottom:"4rem"}}>
                    <Reveal>
                        <Label light>Reference Architecture</Label>
                        <h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.6rem,3vw,2.6rem)",fontWeight:800,lineHeight:1.05,letterSpacing:"-0.03em",color:C.text}}>
                            Commerce Control Plane —{" "}
                            <em style={{fontFamily:"'Instrument Serif',serif",fontStyle:"italic",color:C.blueHi}}>Governance Framework</em>
                        </h2>
                    </Reveal>
                    <Reveal delay={150}>
                        <p style={{fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:"0.82rem",lineHeight:1.85,color:C.muted}}>A reference architecture for governance in AI-driven commerce systems — sitting between commerce platforms and the policies, regulations, and trust standards that govern them.</p>
                    </Reveal>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:"1px",background:C.border,border:`1px solid ${C.border}`}}>
                    {cards.map((c,i)=>(
                        <Reveal key={i} delay={i*80}>
                            <div className="ch" style={{background:C.bg2,padding:"2.5rem",position:"relative",overflow:"hidden",height:"100%"}}>
                                ...
                                <h3 style={{fontFamily:"'Syne',sans-serif",fontSize:"1.05rem",fontWeight:700,color:C.text,letterSpacing:"-0.02em",marginBottom:"0.8rem"}}>{c.title}</h3>
...
const Interest = () => {
    ...
    return (
        <section id="interest" style={{background:C.bg,...SEC}}>
            <div style={INNER}>
                <Label>Why This Matters</Label>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:"5rem",alignItems:"start"}}>
                    <div>
                        <Reveal><h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.6rem,3vw,2.8rem)",fontWeight:800,lineHeight:1.05,letterSpacing:"-0.03em",color:C.text,marginBottom:"1.8rem"}}>
                            Infrastructure for the<br/>
                            <em style={{fontFamily:"'Instrument Serif',serif",fontStyle:"italic",color:C.accent}}>National Interest</em>
                        </h2></Reveal>
                        <Reveal delay={150}><p style={{fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:"0.82rem",lineHeight:1.9,color:C.muted}}>
                            AI systems are rapidly becoming active participants in commerce — setting prices, recommending products, executing transactions, and managing inventory. Without governance infrastructure, these systems introduce systemic risks including regulatory violations, market manipulation, and large-scale consumer harm.<br/><br/>
                        ...
                        <Reveal delay={260}>
                            <div style={{marginTop:"3rem",border:`1px solid ${C.border}`,padding:"2rem"}}>
                                ...
                                <a href="https://github.com/commerce-trust-labs" target="_blank" rel="noreferrer" className="bp" style={{fontSize:"0.68rem",padding:"0.7rem 1.4rem"}}>View on GitHub →</a>
                            </div>
                        </Reveal>
                    </div>
                    <div style={{display:"flex",flexDirection:"column",gap:"1.2rem"}}>
                        {cards.map((c,i)=>(
                            <Reveal key={i} delay={i*100}>
                                <div className="ch" style={{background:C.bg2,border:`1px solid ${C.border}`,padding:"1.8rem 2rem"}}>
...
const Research = () => {
    ...
    return (
        <section id="research" style={{background:C.bg2,...SEC}}>
            <div style={INNER}>
                <Label light>Research & Publications</Label>
                <Reveal><h2 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.6rem,3vw,2.6rem)",fontWeight:800,letterSpacing:"-0.03em",color:C.text,marginBottom:"4rem",lineHeight:1.05}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:"1px",background:C.border,border:`1px solid ${C.border}`}}>
                    {pubs.map((p,i)=>(
                        <Reveal key={i} delay={i*70}>
                            <div className="pr" style={{background:C.bg,padding:"1.8rem 2rem",display:"grid",gridTemplateColumns:"5rem 1fr auto",gap:"1.5rem",alignItems:"center"}}>
                                ...
                                {p.link?<a href={p.link} target="_blank" rel="noreferrer" className="lk">{p.linkLabel}</a>:<span style={{fontFamily:"'DM Mono',monospace",fontSize:"0.68rem",color:C.muted,whiteSpace:"nowrap"}}>{p.linkLabel}</span>}
                            </div>
                        </Reveal>
                    ))}
                </div>
...
/* ── TEAM ─── */
const Team = () => (
    <section id="team" style={{background:C.bg,...SEC}}>
        <div style={INNER}>
            <Label>Founder</Label>
            <Reveal>
                <div style={{display:"grid",gridTemplateColumns:"220px 1fr",gap:"4rem",alignItems:"start",border:`1px solid ${C.border}`,padding:"3.5rem"}}>
                    ...
                    <div style={{minWidth:0}}>
                        <h3 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(1.3rem,2vw,1.9rem)",fontWeight:800,lineHeight:1.1,letterSpacing:"-0.03em",color:C.text,marginBottom:"2rem"}}>
                            Building the{" "}
                        ...
                        ].map((para,i)=><p key={i} style={{fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:"0.79rem",lineHeight:1.9,color:C.muted,marginBottom:"1.2rem"}}>{para}</p>)}
                        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"1px",background:C.border,border:`1px solid ${C.border}`,marginTop:"2rem"}}>
                            {[["Open","Reference Architecture"],["Real-time","Policy Enforcement"],["Federated","Trust Network"],["AI-Native","Governance Layer"]].map(([val,desc])=>(
                                <div key={val} style={{background:C.bg2,padding:"1.2rem"}}>
                                    <div style={{fontFamily:"'Syne',sans-serif",fontSize:"0.95rem",fontWeight:800,color:C.accent,marginBottom:"0.3rem"}}>{val}</div>
...
const Footer = () => (
    <footer style={{background:C.bg2,borderTop:`1px solid ${C.border}`}}>
        <div style={{...INNER,padding:`3.5rem ${PAD}`,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:"2rem",alignItems:"end"}}>
            <div>
                <div style={{fontFamily:"'Syne',sans-serif",fontSize:"1.2rem",fontWeight:800,color:C.text,letterSpacing:"-0.02em",marginBottom:"0.5rem"}}>Commerce Trust Labs</div>
                ...
                <div style={{fontFamily:"'DM Mono',monospace",fontWeight:300,fontSize:"0.66rem",color:"rgba(232,228,220,0.2)",lineHeight:1.8,maxWidth:"44ch"}}>Commerce Trust Labs is an independent research and technology initiative focused on governance infrastructure for AI-driven commerce systems.</div>
            </div>
