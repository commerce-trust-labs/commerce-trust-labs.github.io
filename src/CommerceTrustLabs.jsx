...
const GlobalStyle = () => (
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @keyframes glow { 0%,100% { opacity:0.15 } 50% { opacity:0.05 } }
    ...
    .pub-row:hover { background: #111820 !important; }
...
const Label = ({ children, light }) => (
    <div style={{ display:"flex", alignItems:"center", gap:"1rem", marginBottom:"2.5rem" }}>
        <span style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.25em", textTransform:"uppercase", color: light ? "rgba(232,228,220,0.25)" : "#7a8494", whiteSpace:"nowrap" }}>{children}</span>
        <div style={{ flex:1, height:"1px", background: light ? "rgba(232,228,220,0.06)" : "rgba(232,228,220,0.08)" }} />
    </div>
);
...
const Nav = () => {
    ...
    return (
        <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:200, display:"flex", justifyContent:"space-between", alignItems:"center", padding:"1.1rem 4rem", background: solid ? "rgba(11,15,20,0.97)" : "rgba(11,15,20,0.4)", backdropFilter:"blur(18px)", borderBottom:`1px solid ${solid ? "rgba(232,228,220,0.08)" : "transparent"}`, transition:"all 0.3s" }}>
            <a href="#" style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.7rem", letterSpacing:"0.2em", textTransform:"uppercase", color:"#e8e4dc", textDecoration:"none" }}>
                Commerce Trust Labs
            </a>
...
const Ticker = () => {
    ...
    return (
        <div style={{ background:"#c4501a", overflow:"hidden", padding:"0.6rem 0", whiteSpace:"nowrap" }}>
...
const Network = () => (
    <svg viewBox="0 0 500 500" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width:"100%", height:"100%", maxWidth:"520px" }}>
        <defs>
...
const Hero = () => {
    ...
    return (
        <section style={{ minHeight:"100vh", display:"flex", alignItems:"stretch", background:"#0b0f14", paddingTop:"70px", overflow:"hidden" }}>
            <div style={{ flex:"0 0 50%", display:"flex", flexDirection:"column", justifyContent:"center", padding:"5rem 3rem 5rem 4rem" }}>
                <div style={{ ...anim(80), display:"flex", alignItems:"center", gap:"0.8rem", marginBottom:"1.8rem" }}>
                    <div style={{ width:"2rem", height:"1px", background:"#c4501a" }}/>
                    <span style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.65rem", letterSpacing:"0.22em", textTransform:"uppercase", color:"#c4501a" }}>AI-Governed Commerce Infrastructure</span>
                </div>
                <h1 style={{ ...anim(160), fontFamily:"'Syne',sans-serif", fontSize:"clamp(2.6rem,4.5vw,5rem)", fontWeight:800, lineHeight:1.0, letterSpacing:"-0.03em", color:"#e8e4dc", marginBottom:"1.6rem" }}>
                    The Trust<br/>Layer for<br/>
                    <em style={{ fontFamily:"'Instrument Serif',serif", fontStyle:"italic", fontWeight:400, color:"#c4501a" }}>Modern Commerce</em>
                </h1>
                <p style={{ ...anim(260), fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.85rem", lineHeight:1.85, color:"#7a8494", maxWidth:"42ch", marginBottom:"2.5rem" }}>
                ...
                <div style={{ ...anim(460), display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1px", background:"rgba(232,228,220,0.08)", border:"1px solid rgba(232,228,220,0.08)", marginTop:"4rem" }}>
                    {[["Open","Reference Architecture"],["Real-time","Policy Enforcement"],["AI-Native","Governance Layer"]].map(([val,desc])=>(
                        <div key={val} style={{ background:"#111820", padding:"1.4rem 1.2rem" }}>
                            <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"1.1rem", fontWeight:800, color:"#c4501a", marginBottom:"0.3rem" }}>{val}</div>
                            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.12em", textTransform:"uppercase", color:"#7a8494" }}>{desc}</div>
                        </div>
                    ))}
                </div>
            </div>
            <div style={{ flex:"0 0 50%", display:"flex", alignItems:"center", justifyContent:"center", padding:"4rem 3rem", position:"relative" }}>
                <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at center, rgba(196,80,26,0.06) 0%, transparent 70%)" }}/>
                <Network/>
            </div>
        </section>
    );
...
const Problem = () => {
    ...
    return (
        <section id="problem" style={{ background:"#111820", padding:"7rem 4rem", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"5rem", alignItems:"start" }}>
            <div>
                <Label light>The Problem</Label>
                <Reveal>
                    <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(2rem,3.5vw,3.2rem)", fontWeight:800, lineHeight:1.05, letterSpacing:"-0.03em", color:"#e8e4dc" }}>
                        AI Commerce<br/>Operates Without<br/>
                        <em style={{ fontFamily:"'Instrument Serif',serif", fontStyle:"italic", color:"#c4501a" }}>a Governance Layer</em>
                    </h2>
                </Reveal>
                <Reveal delay={180}>
                    <p style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.8rem", lineHeight:1.85, color:"#7a8494", marginTop:"2rem" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"2rem" }}>
                {problems.map((p,i)=>(
                    <Reveal key={i} delay={i*90}>
                        <div style={{ borderLeft:"2px solid #c4501a", paddingLeft:"1.5rem" }}>
                            <h4 style={{ fontFamily:"'Syne',sans-serif", fontSize:"0.9rem", fontWeight:700, color:"#e8e4dc", marginBottom:"0.4rem" }}>{p.title}</h4>
                            <p style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.76rem", lineHeight:1.75, color:"#7a8494" }}>{p.body}</p>
                        </div>
                    </Reveal>
                ))}
            </div>
...
const Mission = () => {
    ...
    return (
        <section id="mission" style={{ background:"#0b0f14", padding:"7rem 4rem" }}>
            <Label>Mission & Vision</Label>
            <Reveal>
                <p style={{ fontFamily:"'Instrument Serif',serif", fontStyle:"italic", fontSize:"clamp(1.6rem,3vw,2.6rem)", lineHeight:1.35, color:"#e8e4dc", maxWidth:"26ch", marginBottom:"5rem", position:"relative" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1px", background:"rgba(232,228,220,0.08)", border:"1px solid rgba(232,228,220,0.08)" }}>
                {pillars.map((p,i)=>(
                    <Reveal key={i} delay={i*110}>
                        <div className="card-h" style={{ background:"#111820", padding:"2.5rem", height:"100%" }}>
                            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.2em", color:"#c4501a", marginBottom:"1.2rem" }}>{p.n} —</div>
                            <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:"1.05rem", fontWeight:700, color:"#e8e4dc", marginBottom:"0.8rem", letterSpacing:"-0.01em" }}>{p.title}</h3>
                            <p style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.76rem", lineHeight:1.75, color:"#7a8494" }}>{p.body}</p>
                        </div>
                    </Reveal>
                ))}
...
const Technology = () => {
    ...
    return (
        <section id="technology" style={{ background:"#161e28", padding:"7rem 4rem" }}>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"4rem", alignItems:"end", marginBottom:"4rem" }}>
                <Reveal>
                    ...
                    <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(1.8rem,3.5vw,3rem)", fontWeight:800, lineHeight:1.05, letterSpacing:"-0.03em", color:"#e8e4dc" }}>
                        Commerce Control Plane —{" "}
                        <em style={{ fontFamily:"'Instrument Serif',serif", fontStyle:"italic", color:"#6fa3d8" }}>Governance Framework</em>
                    </h2>
                </Reveal>
                <Reveal delay={150}>
                    <p style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.82rem", lineHeight:1.85, color:"#7a8494" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1px", background:"rgba(232,228,220,0.08)", border:"1px solid rgba(232,228,220,0.08)" }}>
                {cards.map((c,i)=>(
                    <Reveal key={i} delay={i*80}>
                        <div className="card-h" style={{ background:"#111820", padding:"3rem", position:"relative", overflow:"hidden", height:"100%" }}>
                            ...
                            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.2em", textTransform:"uppercase", color:"#6fa3d8", marginBottom:"1rem" }}>{c.tag}</div>
                            <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:"1.1rem", fontWeight:700, color:"#e8e4dc", letterSpacing:"-0.02em", marginBottom:"0.8rem" }}>{c.title}</h3>
                            <p style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.76rem", lineHeight:1.75, color:"#7a8494" }}>{c.body}</p>
                        </div>
                    </Reveal>
                ))}
            </div>
...
const Interest = () => {
    ...
    return (
        <section id="interest" style={{ background:"#0b0f14", padding:"7rem 4rem" }}>
            <Label>Why This Matters</Label>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"5rem", alignItems:"start" }}>
                <div>
                    <Reveal>
                        <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(1.8rem,3.5vw,3rem)", fontWeight:800, lineHeight:1.05, letterSpacing:"-0.03em", color:"#e8e4dc", marginBottom:"1.8rem" }}>
                            Infrastructure for the<br/>
                            <em style={{ fontFamily:"'Instrument Serif',serif", fontStyle:"italic", color:"#c4501a" }}>National Interest</em>
                        </h2>
                    </Reveal>
                    <Reveal delay={150}>
                        <p style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.82rem", lineHeight:1.9, color:"#7a8494" }}>
                    ...
                    <Reveal delay={260}>
                        <div style={{ marginTop:"3rem", border:"1px solid rgba(232,228,220,0.08)", padding:"2rem" }}>
                            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.2em", textTransform:"uppercase", color:"#7a8494", marginBottom:"0.8rem" }}>Open Repository</div>
                            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.78rem", color:"#6fa3d8", marginBottom:"0.3rem" }}>commerce-trust-labs /</div>
                            <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"1.1rem", fontWeight:700, color:"#e8e4dc", marginBottom:"0.8rem" }}>commerce-control-plane</div>
                            <div style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.74rem", color:"#7a8494", lineHeight:1.7, marginBottom:"1.2rem" }}>Reference architecture for AI-native governance layers in distributed commerce infrastructure.</div>
                            <div style={{ display:"flex", gap:"1.2rem", flexWrap:"wrap", marginBottom:"1.5rem" }}>
                                {["Java / Spring","Open Source"].map(t=>(<span key={t} style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.66rem", color:"#7a8494" }}>◆ {t}</span>))}
                                <span style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.66rem", color:"#c4501a" }}>◆ Active</span>
                            </div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"1.2rem" }}>
                    {cards.map((c,i)=>(
                        <Reveal key={i} delay={i*100}>
                            <div className="card-h" style={{ background:"#111820", border:"1px solid rgba(232,228,220,0.08)", padding:"1.8rem 2rem" }}>
                                <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.58rem", letterSpacing:"0.2em", textTransform:"uppercase", color:"#c4501a", marginBottom:"0.5rem" }}>{c.tag}</div>
                                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"0.9rem", color:"#e8e4dc", marginBottom:"0.4rem" }}>{c.title}</div>
                                <div style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.74rem", color:"#7a8494", lineHeight:1.7 }}>{c.body}</div>
                            </div>
                        </Reveal>
                    ))}
                </div>
...
const Research = () => {
    ...
    return (
        <section id="research" style={{ background:"#111820", padding:"7rem 4rem" }}>
            <Label light>Research & Publications</Label>
            <Reveal>
                <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(1.8rem,3.5vw,3rem)", fontWeight:800, letterSpacing:"-0.03em", color:"#e8e4dc", marginBottom:"4rem", lineHeight:1.05 }}>
                    Intellectual Contributions{" "}
                    <em style={{ fontFamily:"'Instrument Serif',serif", fontStyle:"italic", color:"#c4501a" }}>to the Field</em>
                </h2>
            </Reveal>
            <div style={{ display:"flex", flexDirection:"column", gap:"1px", background:"rgba(232,228,220,0.08)", border:"1px solid rgba(232,228,220,0.08)" }}>
                {pubs.map((p,i)=>(
                    <Reveal key={i} delay={i*70}>
                        <div className="pub-row" style={{ background:"#0b0f14", padding:"2rem 2.5rem", display:"grid", gridTemplateColumns:"5.5rem 1fr auto", gap:"2rem", alignItems:"center" }}>
                            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.58rem", letterSpacing:"0.15em", textTransform:"uppercase", color:"#c4501a" }}>{p.type}</div>
                            <div>
                                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"0.9rem", color:"#e8e4dc", letterSpacing:"-0.01em", marginBottom:"0.3rem" }}>{p.title}</div>
                                <div style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.72rem", color:"#7a8494", lineHeight:1.6 }}>{p.desc}</div>
                            </div>
                            {p.link
                                ? <a href={p.link} target="_blank" rel="noreferrer" className="lnk">{p.linkLabel}</a>
                                : <span style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.68rem", color:"#7a8494", whiteSpace:"nowrap" }}>{p.linkLabel}</span>
                            }
                        </div>
                    </Reveal>
                ))}
...
const Team = () => (
    <section id="team" style={{ background:"#0b0f14", padding:"7rem 4rem" }}>
        <Label>Founder</Label>
        <Reveal>
            <div style={{ display:"grid", gridTemplateColumns:"240px 1fr", gap:"5rem", alignItems:"start", border:"1px solid rgba(232,228,220,0.08)", padding:"4rem" }}>
                <div>
                    <div style={{ width:"100%", aspectRatio:"3/4", background:"#111820", border:"1px solid rgba(232,228,220,0.08)", marginBottom:"1.5rem", position:"relative", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden" }}>
                        <span style={{ fontFamily:"'Syne',sans-serif", fontSize:"4rem", fontWeight:800, color:"#e8e4dc", opacity:0.07 }}>PS</span>
                        <div style={{ position:"absolute", bottom:0, left:0, right:0, height:"3px", background:"#c4501a" }}/>
                    </div>
                    <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"1.1rem", fontWeight:700, color:"#e8e4dc", marginBottom:"0.3rem" }}>Pranesh Soma</div>
                    <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.65rem", letterSpacing:"0.12em", textTransform:"uppercase", color:"#c4501a" }}>Founder & Chief Architect</div>
                </div>
                <div>
                    <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(1.4rem,2.2vw,2rem)", fontWeight:800, lineHeight:1.1, letterSpacing:"-0.03em", color:"#e8e4dc", marginBottom:"2rem" }}>
                        Building the{" "}
                    ...
                    {[
                        ...
                        <p key={i} style={{ fontFamily:"'DM Mono',monospace", fontWeight:300, fontSize:"0.79rem", lineHeight:1.9, color:"#7a8494", marginBottom:"1.2rem" }}>{para}</p>
                    ))}
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"1px", background:"rgba(232,228,220,0.08)", border:"1px solid rgba(232,228,220,0.08)", marginTop:"2rem" }}>
                        {[["Open","Reference Architecture"],["Real-time","Policy Enforcement"],["Federated","Trust Network"],["AI-Native","Governance Layer"]].map(([val,desc])=>(
                            <div key={val} style={{ background:"#111820", padding:"1.2rem" }}>
                                <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"1rem", fontWeight:800, color:"#c4501a", marginBottom:"0.3rem" }}>{val}</div>
                                <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.58rem", letterSpacing:"0.1em", textTransform:"uppercase", color:"#7a8494" }}>{desc}</div>
                            </div>
                        ))}
                    </div>
                </div>
...
const Footer = () => (
    <footer style={{ background:"#111820", borderTop:"1px solid rgba(232,228,220,0.08)", padding:"4rem 4rem", display:"grid", gridTemplateColumns:"1fr 1fr", alignItems:"end", gap:"2rem" }}>
        <div>
            <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"1.3rem", fontWeight:800, color:"#e8e4dc", letterSpacing:"-0.02em", marginBottom:"0.5rem" }}>Commerce Trust Labs</div>
            <div style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.68rem", color:"#7a8494", marginBottom:"1.2rem" }}>AI-Governed Commerce Infrastructure — Cumming, Georgia, USA</div>
